#!/usr/bin/env python3
"""
Lindley Periodic Variance Model Simulation Runner

This file runs simulations using the periodic variance model and saves all data to files.
Uses LindleyPeriodicVarModel.py functions for parameter estimation.

Usage:
- Test run: python run_periodic_var.py --test
- Full study: python run_periodic_var.py --full
- Custom: python run_periodic_var.py --n_sims 50 --n_years 50
...
from LindleyPeriodicVarModel import LindleyPeriodicVarModel, log_likelihood_periodic_var_optimized, estimate_parameters_periodic_var_optimized
...
def run_periodic_var_study(n_simulations=100, n_years=100, save_results=True, results_folder=None, model_type="periodic_var", mute=False):
    """
    Run the complete periodic variance simulation study:
    
    Process:
    1. Simulate one 100-year series using Lindley recursion with periodic variance
    2. Obtain parameter estimates from this 100-year series (8 parameters)
    3. Store the parameter estimates and the C_t/X_t datasets
    4. Repeat this entire process n_simulations times
    ...
    - model_type: Type of model ("periodic_var")
    - mute: If True, show only progress bar instead of detailed output
    
    Result:
    - n_simulations different n_years-year time series (C_t and X_t)
    - n_simulations parameter estimates (one from each time series)
    ...
    # True parameters for periodic variance model (8 parameters)
    true_params = {
        'alpha0': -0.7,
        'alpha1': 1.4,
        'tau1': 355,
    ...
    model = LindleyPeriodicVarModel(**true_params)
    param_names = ['alpha0', 'alpha1', 'tau1', 'alpha2', 'beta0', 'beta1', 'tau2']
    
    # Store results
    estimated_params = []
    success_flags = []
    all_series_data = []  # Store all time series
    ...
    for sim in range(n_simulations):
        ...
        # Step 1: Generate one complete time series (C_t and X_t)
        ...
        # Step 3: Estimate parameters from this complete time series
        if not mute:
            print("  Estimating parameters...", end="", flush=True)
        
        try:
            params_est, success = estimate_parameters_periodic_var_optimized(depths, changes)
            ...
            # Add NaN values to maintain array structure (8 parameters)
            estimated_params.append([np.nan] * 8)
    ...
    # Calculate and display summary statistics for 8 parameters
    if not mute:
    ...
    if not mute:
        ...
        print("="*120)
        print(f"{'Sim':<4} | {'Success':<8} | {'alpha0':<10} | {'alpha1':<10} | {'tau1':<8} | {'alpha2':<12} | {'beta0':<10} | {'beta1':<10} | {'tau2':<8} | {'beta2':<12}")
        print("-"*120)
        ...
        for i in range(min(5, len(results_df))):
            ...
            if success and not results_df.iloc[i][param_names].isna().any():
                ...
                print(f"{sim_num:<4} | {'True':<8} | {params[0]:<10.4f} | {params[1]:<10.4f} | {params[2]:<8.1f} | {params[3]:<12.8f} | {params[4]:<10.4f} | {params[5]:<10.4f} | {params[6]:<8.1f} | {params[7]:<12.8f}")
            else:
                print(f"{sim_num:<4} | {'False':<8} | {'NaN':<10} | {'NaN':<10} | {'NaN':<8} | {'NaN':<12} | {'NaN':<10} | {'NaN':<10} | {'NaN':<8} | {'NaN':<12}")
    ...
    if save_results:
        ...
        with open(summary_file, 'w') as f:
            f.write("LINDLEY PERIODIC VARIANCE MODEL SIMULATION STUDY RESULTS\n")
            f.write("=" * 60 + "\n")
            ...
            f.write("PARAMETER ESTIMATION RESULTS (8 Parameters)\n")
            f.write("-" * 45 + "\n")
            f.write("Mean parameters: α₀, α₁, τ₁, α₂\n")
            f.write("Variance parameters: β₀, β₁, τ₂, β₂\n")
            f.write("\n")
        ...
        with open(config_file, 'w') as f:
            f.write("PERIODIC VARIANCE SIMULATION CONFIGURATION\n")
            f.write("-" * 40 + "\n")
            ...
            f.write(f"Parameters estimated: 7\n")
            f.write(f"Mute mode: {mute}\n")
            f.write("\nTRUE PARAMETERS (7 Total)\n")
            f.write("-" * 30 + "\n")
            f.write("MEAN PARAMETERS:\n")
            for param in ['alpha0', 'alpha1', 'tau1', 'alpha2']:
                f.write(f"  {param}: {true_params[param]}\n")
            f.write("VARIANCE PARAMETERS:\n")
            for param in ['beta0', 'beta1', 'tau2']:
                f.write(f"  {param}: {true_params[param]}\n")
...
def main():
    parser = argparse.ArgumentParser(description='Run Lindley Periodic Variance Model Simulations')
    ...
    parser.add_argument('--model_type', type=str, default='periodic_var',
                       choices=['periodic_var'],
                       help='Model type for folder naming (default: periodic_var)')
    ...
    if args.test:
        n_simulations = 1
        n_years = 100
        print("\nRUNNING TEST SIMULATION (PERIODIC VARIANCE)")
        print("=" * 50)
    elif args.full:
        n_simulations = 100
        n_years = 100
        print("\nRUNNING FULL SIMULATION STUDY (PERIODIC VARIANCE)")
        print("=" * 50)
    elif args.n_sims is not None:
        n_simulations = args.n_sims
        n_years = args.n_years
        print(f"\nRUNNING CUSTOM SIMULATION (PERIODIC VARIANCE)")
        print("=" * 50)
    else:
    ...
    print(f"• Parameters estimated: 8 (α₀, α₁, τ₁, α₂, β₀, β₁, τ₂, β₂)")
    ...
    if n_simulations >= 50 and not args.mute:
        response = input(f"This will run {n_simulations} periodic variance simulations. Continue? (y/n): ")
        if response.lower() not in ['y', 'yes']:
            print("Simulation cancelled.")
            return
    
    # Run simulation
    print("Starting periodic variance simulation...")
    results = run_periodic_var_study(
        n_simulations=n_simulations,
        n_years=n_years,
    ...
    if not args.no_save and results['results_folder']:
        ...
        with open(ref_file, 'w') as f:
            f.write("PERIODIC VARIANCE SIMULATION RUN INFORMATION\n")
            f.write("=" * 45 + "\n")
            f.write(f"Command: python run_periodic_var.py")
            if args.test:
                f.write(" --test")
            elif args.full:
                f.write(" --full")
            ...
            f.write(f"Parameters: 7 (periodic variance model)\n")
